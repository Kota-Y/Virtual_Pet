...
//          Input.location.Start ();
	...
	public void Calc (float lat1, float lon1, float lat2, float lon2)
	{
		...
		//convert distance from double to float, 距離をdouble型からfloat型へ変換
		float distanceFloat = (float)distance;
		//set the target position of the dog, this is where we lerp to in the update function
		// 犬のターゲット地点を設定、アップデート関数で犬の位置を表示
